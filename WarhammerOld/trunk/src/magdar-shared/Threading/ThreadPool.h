...
public:
	...
	__forceinline uint32 GetId() { return (uint32)thread_id; }
};
...
public:
	...
	__forceinline void Gobble() { _threadsEaten=(int32)m_freeThreads.size(); }

	// gets active thread count
	__forceinline uint32 GetActiveThreadCount() { return (uint32)m_activeThreads.size()+1; }

	// gets free thread count
	__forceinline uint32 GetFreeThreadCount() { return (uint32)m_freeThreads.size(); }
};
