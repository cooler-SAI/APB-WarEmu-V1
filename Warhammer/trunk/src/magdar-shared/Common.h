...
__forceinline void reverse_array(uint8 * pointer, size_t count)
{
	size_t x;
...
__forceinline void HEARTHSTONE_TOLOWER(std::string& str)
{
	for(size_t i = 0; i < str.length(); ++i)
		str[i] = (char)tolower(str[i]);
};

__forceinline void HEARTHSTONE_TOUPPER(std::string& str)
{
	for(size_t i = 0; i < str.length(); ++i)
