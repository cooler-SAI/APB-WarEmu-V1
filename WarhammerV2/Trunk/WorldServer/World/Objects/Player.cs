...
namespace WorldServer
{
    public class Player : Unit
    {
        ...
        static public int DISCONNECT_TIME = 3000;
        ...
        static public void RemovePlayer(Player Plr)
        {
            lock (_Players)
            {
                if (_Players.Remove(Plr))
                {
                    ...
                    CharMgr.Database.ExecuteNonQuery("UPDATE characters SET Online=0 WHERE CharacterId=" + Plr._Info.CharacterId +";");
                    Plr._Info.Online = 0;
                    Program.Rm.OnlinePlayers = (uint)_Players.Count;
                    Program.AcctMgr.UpdateRealm(Program.Rm.RealmId, Program.Rm.OnlinePlayers, OrderCount, DestruCount);
                }
        ...
        public MailsInterface MlsInterface;
        ...
        public Player(GameClient Client,Character Info) : base()
        {
            ...
            Name = Info.Name;
            Realm = (GameData.Realms)_Info.Realm;
            SetPVPFlag(false);
            ...
            MlsInterface = AddInterface(CharMgr.GetMails(_Info.CharacterId, true)) as MailsInterface;
        ...
        public override void OnLoad()
        {
            ...
            if (!_Inited)
            {
                ...
                _Inited = true;
                CharMgr.Database.ExecuteNonQuery("UPDATE characters SET Online=1 WHERE CharacterId=" + _Info.CharacterId + ";");
            }
        ...
        public void StartInit()
        {
            ...
            if (GrpInterface.IsInGroup())
                GrpInterface.CurrentGroup.SendGroupCharacters();

            SendMessage(0, "Welcome to warhammer private server 1.4.8", "", SystemData.ChatLogFilters.CHATLOGFILTERS_CITY_ANNOUNCE);
            MailsMgr.SendMailsCount(this);
        }
        ...
        public void PreRespawnPlayer()
        {
            ...
            if (!EvtInterface.HasEvent(RespawnPlayer))
            {
                ...
                EvtInterface.AddEvent(RespawnPlayer, 5000, 1); // 5 Seconds to Respawn (Official Time)
            }
        }
        ...
        public void LevelUp(uint RestXp)
        {
            ...
            //SendMessage(0, "Server", "You have level up.Due to a bug, please disconnect and reconnect for update your level.", SystemData.ChatLogFilters.CHATLOGFILTERS_SAY);
            if (CurrentXp == null)
                return;
