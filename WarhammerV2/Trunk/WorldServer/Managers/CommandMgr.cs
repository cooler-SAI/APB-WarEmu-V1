...
namespace WorldServer
{
    ...
    static public class CommandMgr
    {
        ...
        static public void PartyInvite(Player Plr, string Text)
        {
            //Log.Info("Invite", Text);

            Player Target = Player.GetPlayer(Text);
            GameData.Localized_text Result = Plr.GrpInterface.CanInvite(Target);
            if (Result == GameData.Localized_text.TEXT_GROUP_INVITE_BEGIN)
            {
                Target.GrpInterface.ReceiveInvitation(Plr);
            }

            Plr.SendLocalizeString(Text, Result);
        }
        ...
        {
            //Log.Info("PartySay", Text);
            Plr.GrpInterface.Say(Text);
        }
