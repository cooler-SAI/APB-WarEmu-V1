...
namespace WorldServer
{
    ...
    static public class GmCommand
    {
        ...
        static public List<GmCommandHandler> ModifyCommands = new List<GmCommandHandler>()
        {
            ...
            new GmCommandHandler("renown",ModifyRenown, null, 3, 1, "Change the speed of selected Playe <Level>r"),
        ...
        static public List<GmCommandHandler> BaseCommand = new List<GmCommandHandler>()
        {
            ...
            new GmCommandHandler("testtest", Test, null, 3, 2, "Database Commands"),
        };

        static public bool Test(Player Plr, ref List<string> Values)
        {
            int id=  GetInt(ref Values);
            int t = GetInt(ref Values);

            /*{
                string Str = GetString(ref Values);
                string SubStr = GetString(ref Values);

                PacketOut Out = new PacketOut((byte)Opcodes.F_INIT_EFFECTS);
                Out.WriteHexStringBytes(Str);
                //Out.WriteByte(0x01);
                //Out.WriteByte(01);
                //Out.WriteUInt16(0);
                ...
                Out.WriteByte(2);
                Out.WriteByte(0);
                Out.WriteUInt16R(1932);
                Out.WriteHexStringBytes(SubStr);

                //Out.WriteByte(2);
                //Out.WriteByte(3);
                //Out.WriteUInt16(0x0101);
                //Out.WriteByte(0xFB);
            ...
            {*/
                PacketOut Out = new PacketOut((byte)Opcodes.F_INIT_EFFECTS);
                Out.WriteHexStringBytes("01 01 00 00".Replace(" ", string.Empty));
                Out.WriteUInt16(Plr.Oid);
                Out.WriteUInt16(0x0281);
                Out.WriteUInt16R(1932); // Entry
                Out.WriteByte((byte)(id*2)); // Time * 2 ?
                Out.WriteByte((byte)t);
                Out.WriteUInt16(0x81);
                Out.WriteUInt16(0x0201);
            ...
            if (id == 3)
            {
                ...
                Out.WriteHexStringBytes("01 01 00 00".Replace(" ", string.Empty));
                Out.WriteUInt16(Plr.Oid);
                Out.WriteHexStringBytes("0B 00 35 01 00 81 02 01 00 88 02 00".Replace(" ", string.Empty));
                Plr.SendPacket(Out);
            }*/
        ...
        static public bool NpcGoTo(Player Plr, ref List<string> Values)
        {
            ...
            if (T != null)
            {
                ...
                /*{
                    ...
                    Out.WriteHexStringBytes("50 0C D1 49 1B B0 64 10 6A 00 00 00 00 00 B0 0D 01 00 00".Replace(" ", string.Empty));
                    Plr.SendPacket(Out);
                }*/
                ...
                /* {
                     PacketOut Out = new PacketOut((byte)Opcodes.F_OBJECT_STATE);
                     Out.WriteUInt16(T.Oid);
                     Out.WriteHexStringBytes("5810B0E01BCC1C006A0000000000BC00");
                     Plr.SendPacket(Out);
                 }

                 {
                     PacketOut Out = new PacketOut((byte)Opcodes.F_GRAPHICAL_REVISION);
                     Out.WriteUInt16(T.Oid);
                     Out.WriteByte(1);
                     Out.WriteByte(0);
                     Plr.SendPacket(Out);
               ...
               /* {
                    PacketOut Out = new PacketOut((byte)Opcodes.F_OBJECT_STATE);
                    Out.WriteUInt16(T.Oid);
                    Out.WriteUInt16((ushort)T.X);
                    Out.WriteUInt16((ushort)T.Y);
                    Out.WriteUInt16((ushort)T.Z);
                    Out.WriteByte(100);
                    Out.WriteByte(3);
                    Out.WriteByte((byte)T.Zone.ZoneId);
                    ...
                    Log.Dump("TPackets", Out, true);
                    Out.WritePacketLength();
                    Plr.SendPacket(Out);
                }

                Log.Info("Movement", "Start=" + T.X + "," + T.Y + "," + T.Z + ",End=" + X + "," + Y + "," + Z);*/
